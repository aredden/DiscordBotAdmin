...
export default class InputBox extends Component<InputBoxProps, InputBoxState> {
	...
	handleEmojiChooseNoEvent(emojiTag: string) {
		...
		this.setState({
			content:
				colonMatch !== null
					? content.replace(new RegExp(`${colonMatch}$`, 'g'), '') + emojiTag + ' '
					: content + emojiTag + ' ',
			colonMatch: null,
		});
	...
	handleUpPress(e) {
		if (
			(e.key === 'ArrowUp' || e.key === 'Tab' || e.key === 'Enter') &&
			!isNullOrUndefined(
				document.getElementById('transitions-popper-autocomplete')
			)
		) {
			e.preventDefault();
	...
	render() {
		...
		if (emojis) {
			...
			let regex = colonMatch
				? new RegExp(`^${colonMatch.replace(':', '')}`, 'g')
				: /^noword/;
			let autoCompleteData = {
				emojis: Object.values(emojis).filter(
					(emoji: TypeEmoji) => emoji.name.match(regex) !== null
				),
...
class EmojiAutoComplete extends Component<EmojiAutoCompleteProps> {
		...
		switch (event.key) {
			...
			case 'ArrowDown':
				...
				if (idx - 1 >= 0) {
					...
					document.getElementById(`emojiautocomplete-${emojis.length - 1}`).focus();
				break;
			case 'Enter':
				event.preventDefault();
				event.stopPropagation();
				onChoose(
					`<${emojis[idx].url.match(/.gif$/gm) !== null ? 'a' : ''}:${
						emojis[idx].name
					}:${emojis[idx].id}>`
				);
				break;
	...
	render() {
		...
		return (
			<Popper
				className="container col-md-3 card bg-light p-3"
				id={id}
				open={open}
...
const EmojiNames = (
	emojis: TypeEmoji[],
	handleKeyboardEvent: (event, index: number) => void,
	handleClickChoose: (event, tag) => any
) => {
	return (
		<form
			...
			{emojis
				.map((emoji, idx) => {
					...
					return (
						<button
							onKeyDown={(e) => handleKeyboardEvent(e, idx)}
							onClick={(e) =>
								handleClickChoose(
									e,
									`<${emoji.url.match(/.gif$/gm) !== null ? 'a' : ''}:${emoji.name}:${
										emoji.id
									}>`
								)
							}
...
const EmojiWindow = (
	emojis: EmojiMap,
	emojiChooser: (e: MouseEvent, emojiTag: string) => void,
	columns: number
) => {
	return (
...
function EmojiIcon(
	id: string,
	emoji: TypeEmoji,
	emojiChooser: (e: MouseEvent, emojiTag: string) => any
) {
	return (
		<button
			...
			onClick={(e) =>
				emojiChooser(
					e,
					`<${emoji.url.match(/.gif$/) !== null ? 'a' : ''}:${emoji.name}:${
						emoji.id
					}>`
				)
			}>
