			...
			<nav className="col-md-2 pr-0 shadow-lg navbar-default navbar-dark bg-dark sidebar">
				<div className="sidebar-sticky pr-0 mr-0">
					<ul className="nav flex-column">
						...
						{guildList
							? Object.values(guildList).map((guild: TypeGuild) => {
									return (
										<li
											className="nav-item btn btn-dark d-flex justify-content-start py-0"
					...
					<li>
						...
						<h5
							className="sidebar-heading d-flex justify-content-between
                                align-items-center px-4 mt-3 mb-1"
							style={headingColor}>
							<span>{guildName ? guildName : 'Waiting...'}</span>
						</h5>
						<hr />
					</li>
					<ul className="nav flex-column mb-2">
						{guildChannels
							? Object.values(guildChannels).map((channel) => {
									let num = notifications[guildName + channel.name];
									return (
										<li
											className="nav-item d-flex justify-content-start btn btn-dark py-0"
											onClick={(e) => onSwitchChannel(e, channel.name)}
											key={channel.id}>
											<div className="nav-link" key={channel.id}>
												<div className="d-flex display-inline">
													<div
														className="text-truncate"
														style={{ fontSize: 16, maxWidth: '9rem', color: '#bbb' }}>
														#{channel.name}
													</div>
													&nbsp;
													{num > 0 ? (
