...
const EmbedTitle = ({ title, url }) => {
	...
	let computed = (
		<div className="embed-title">{parseEmbedTitle(parseForNewline(title))}</div>
	);
	if (url) {
		computed = (
			<Link href={url} className="embed-title">
				{parseEmbedTitle(parseForNewline(title))}
			</Link>
		);
	}
...
const EmbedAuthor = ({ name, avatarURL, avatar }: TypeUser) => {
	...
	if (name) {
		...
		if (avatarURL) {
			...
			authorName = (
				<Link href={avatarURL} className="embed-author-name">
					{name}
				</Link>
			);
		}
	...
	const authorIcon = avatarURL ? (
		<img src={avatarURL} alt="" className="embed-author-icon" />
	) : null;
...
const EmbedField = ({ name, value, inline }) => {
	...
	const fieldName = name ? (
		<div className="embed-field-name">
			{parseEmbedTitle(parseForNewline(name))}
		</div>
	) : null;
	const fieldValue = value ? (
		<div
			key={`${name}-${Math.floor(Math.random() * 10000)}`}
			className="embed-field-value markup">
			{parseAllowLinks(parseForNewline(value))}
		</div>
	) : null;

	return (
		<div key={Math.floor(Math.random() * 10000)} className={cls}>
			{fieldName}
			{fieldValue}
		</div>
	);
