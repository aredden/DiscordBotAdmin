...
export async function parseGuilds(
	guilds: GuildManager
): Promise<Map<string, TypeGuild>> {
...
): Promise<Map<string, TypeTextChannel>> {
	let channelList: Map<string, TypeTextChannel> = new Map<
		string,
		TypeTextChannel
	>();
...
export async function parseTextChannel(
	channel: TextChannel
): Promise<TypeTextChannel> {
...
export async function parseNewMessage(
	...
	let edits = await Promise.all(
		message.edits.map(
			async (_message: Message) => await parseMessageNoEdits(_message)
		)
	);
...
export async function parseMessageNoEdits(
	message: Message | PartialMessage
): Promise<TypeMessageNoEdits> {
	const txtchannel = message.channel as TextChannel;
 ...
 */
export async function parseGuildMembers(
	members: Collection<string, GuildMember>
): Promise<Map<string, TypeGuildMember>> {
	let memberMap: Map<string, TypeGuildMember> = new Map<
		string,
		TypeGuildMember
	>();
...
export async function parseMessageReactions(
	reactions: Collection<string, MessageReaction>
): Promise<TypeMessageReaction[]> {
	let reactionArray = Array.from(reactions);
	let reactionAsyncArray: TypeMessageReaction[] = new Array<
		TypeMessageReaction
	>();
...
export async function parseMessageReaction(
