 ...
 * @param emojiString
 */
function getEmojisAfterConfirmed(
	emojiString: string,
	...
	emojiNameArray.forEach((element, index) => {
		if (!emojiMap[element]) {
			...
			let emoji: TypeEmoji = getTypeEmojiFromParsedEmojiWord(
				emojiName,
				emojiId,
				emojiUrl
			);
 ...
 * @param url
 */
function getTypeEmojiFromParsedEmojiWord(
	name: string,
	id: string,
	url: string
): TypeEmoji {
 ...
 * @param emojis
 */
export function parseEmojisFromString(
	emojiString: string,
	emojis: Map<string, TypeEmoji>
): Map<string, TypeEmoji> {
 ...
 * @param currentEmojis
 */
export function concatEmojiCollections(
	emojis: emojiColl,
	currentEmojis: emojiColl
): emojiColl {
