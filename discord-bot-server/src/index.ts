...
export function updateEmojiMap(
	emojis: Map<string, TypeEmoji>,
	toRemove?: Map<string, TypeEmoji>
) {
	logger.info(
		chalk.yellow('ATTEMPTING TO UPDATE EMOJIS:') + JSON.stringify(emojis, null, 1)
	);
	if (toRemove) {
	...
	logger.info(
		chalk.yellow('UPDATED EMOJIS:') + JSON.stringify(EMOJIS_MAP, null, 1)
	);
	return EMOJIS_MAP;
}
...
export function updateChannelNotifications(
	key: string,
	value?: number,
	newChannel?: boolean
) {
...
bot.client.once('ready', async () => {
	...
	GUILD_DATA = await parseGuilds(bot.client.guilds);
	KEY_FOCUS = 'Zippys Mega Servergeneral';
