...
export default function lmaoSocket(bot: DiscordBot, server: http.Server) {
  ...
  bot.client.once("ready", () => {
    ...
    io.on("connection", (socket: Socket) => {
      ...
      const guildMemberChangeListener = async (
        member: GuildMember | PartialGuildMember
      ) => {
        let memb = member.partial
          ? await member.fetch()
          : (member as GuildMember);
        handleMemberUpdate(memb, socket);
      };

      const guildMemberUpdateListener = async (
        _oldMember: GuildMember | PartialGuildMember,
        newMember: GuildMember | PartialGuildMember
      ) => {
        let memb = newMember.partial
          ? await newMember.fetch()
          : (newMember as GuildMember);
        handleMemberUpdate(memb, socket);
      };

      const messageReactionAddListener = (
      ...
      const messageReactionRemoveListener = (
        reaction: MessageReaction,
        _user: User | PartialUser
      ) => {
      ...
      const messageDeleteListener = async (
        message: Message | PartialMessage
      ) => {
        let msg = message.partial
          ? await message.fetch()
          : (message as Message);
      ...
      socket.on("updateGuild", async (updateArguments: string) => {
        handleUpdateGuildRequest(
          JSON.parse(updateArguments) as UpdateGuildArguments,
          socket
        );
      });
      ...
      socket.on("updateChannel", (updateArguments: string) => {
        handleUpdateChannelRequest(
          JSON.parse(updateArguments) as UpdateChannelArguments,
          socket
        );
      });
      ...
      socket.on("updateMember", (updateArguments: string) => {
        handleUpdateMemberRequest(
          JSON.parse(updateArguments) as UpdateMemberArguments,
          socket
        );
      });
      ...
      socket.on("updateMessage", (updateArguments: string) => {
        handleUpdateMessageRequest(
          JSON.parse(updateArguments) as UpdateMessageArguments,
          socket
        );
      });
      ...
      socket.on("requestMessages", (jsonReqParams: string) => {
        let { guildID, channelID, lastMessage } = JSON.parse(jsonReqParams);
